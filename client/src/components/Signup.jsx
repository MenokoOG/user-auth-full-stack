...
const Signup = () => {
  ...
  const { values, handleChange, handleSubmit } = useForm(async () => {
    try {
      ...
      await apiClient.post('/api/auth/login', { username: values.username, password: values.password });
      navigate('/');
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-background">
      <div className="w-full max-w-md p-8 bg-card rounded-lg shadow-custom border border-border">
        <h2 className="text-3xl font-semibold text-primary mb-6 text-center">Signup</h2>
